...
const tile=(item,index)=>`<button class="gallery-tile gallery-${item.cat}" data-index="${index}" aria-label="Open ${item.label}"><img loading="lazy" src="${mediaPath(item.file)}" alt="${item.label}"><span class="tile-label">${item.label}</span><span class="tile-category">${item.cat==='computer-science'?'Computer Science':'Robotics'}</span></button>`;
...
render();
document.querySelectorAll('.filter-bar button').forEach(button=>button.addEventListener('click',()=>{document.querySelector('.filter-bar .active').classList.remove('active');document.querySelectorAll('.filter-bar button').forEach(item=>item.setAttribute('aria-selected','false'));button.classList.add('active');button.setAttribute('aria-selected','true');render(button.dataset.filter)}));
...
const toggle=document.querySelector('.menu-toggle'),menu=document.querySelector('.mobile-menu'),closeMenu=()=>{menu.classList.remove('open');menu.setAttribute('aria-hidden','true');toggle.setAttribute('aria-expanded','false');document.body.classList.remove('menu-open')},openMenu=()=>{menu.classList.add('open');menu.setAttribute('aria-hidden','false');toggle.setAttribute('aria-expanded','true');document.body.classList.add('menu-open')};toggle.addEventListener('click',()=>menu.classList.contains('open')?closeMenu():openMenu());menu.querySelector('.menu-close').addEventListener('click',closeMenu);menu.querySelectorAll('a').forEach(a=>a.addEventListener('click',closeMenu));document.addEventListener('keydown',e=>{if(e.key==='Escape'&&menu.classList.contains('open'))closeMenu()});
